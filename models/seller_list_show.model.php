...
//     return $statement->rowCount() > 0;
// }
function editShow($dateShow, $hallName, $timeStart, $id) {
    global $connection;
    // ensure that $id is an integer
    var_dump($id); // add this line
    if (!is_numeric($id)) {
        throw new InvalidArgumentException('The $id parameter must be numeric.');
    }
    $id = (int) $id;

    // prepare the SQL query
    $sql = "UPDATE hall_shows SET date_show = ?, hall_id = ?, time_start = ? WHERE show_id = ?";

    // execute the query with bound parameters
    $stmt = $connection->prepare($sql);
    $stmt->bindParam(1, $dateShow);
    $stmt->bindParam(2, $hallName);
